...
class Gmap_Service_Googlemap extends Phpfox_Service 
{
	...
	/**Center of the gmap **/
	protected $center = 'United States';
	...
	/** Http **/
	protected $protocolUrl = 'http';
	
	protected $customLocationLat = 0;
	protected $customLocationLng = 0;
	protected $customViewportNLat = 0;
	protected $customViewportNLng = 0;
	protected $customViewportSLat = 0;
	protected $customViewportSLng = 0;
	...
	public function __construct() {
		$this->protocolUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on' ? 'https' : 'http');
		$this->clustererLibrarypath = $this->protocolUrl.'://google-maps-utility-library-v3.googlecode.com/svn/tags/markerclusterer/1.0/src/markerclusterer_packed.js';
	}

	public function setCustomLocationAndViewport($aInput)
	{
		$this->customLocationLat = $aInput['lat'];
		$this->customLocationLng = $aInput['lng'];
		$this->customViewportNLat = $aInput['northeast_lat'];
		$this->customViewportNLng = $aInput['northeast_lng'];
		$this->customViewportSLat = $aInput['southwest_lat'];
		$this->customViewportSLng = $aInput['southwest_lng'];
	}
	
	public function setDefaultMarker($username)
	{
		$this->defaultMarker = $username;
	}
	...
	public function geocoding($address) {
		$url = $this->protocolUrl.'://maps.googleapis.com/maps/api/geocode/json?address=' . urlencode($address) . '&sensor=true';
	 ...
	 * @return void
	 */
	public function addMarkerByCoords($lat, $lng, $title, $html = '', $username = '', $category = '', $icon = '') {
		/*if ($icon == '') {
			$icon = $this->protocolUrl.'://maps.gstatic.com/mapfiles/markers2/marker.png';
		}*/
		...
		$this->contentMarker .= "\t" . 'addMarker(new google.maps.LatLng("' . $lat . '","' . $lng . '"),"' . $title . '","' . $html . '","' . $username . '","' . $category . '","' . $icon . '",map' . $this->googleMapId . ');' . "\n";
	}
	 ...
	 * @return void
	 */
	public function addMarkerByAddress($address, $title = '', $content = '', $username = '', $category = '', $icon = '') {
		$point = $this->geocoding($address);
		if ($point !== NULL) {
			$this->addMarkerByCoords($point[2], $point[3], $title, $username, $category, $icon);
		} else {
	...
	public function addArrayMarkerByCoords($coordtab, $category = '', $icon = '') {
		foreach ($coordtab as $coord) {
			$this->addMarkerByCoords($coord[0], $coord[1], $coord[2], $coord[3], $coord[4], $category, $icon);
		}
	}
	...
	public function init() {
		...
		if ($this->includeJs === TRUE) {
			// Google map JS
			$this->content .= '<script type="text/javascript" src="'.$this->protocolUrl.'://maps.google.com/maps/api/js?sensor=false&language=' . $this->lang . '">';
			$this->content .= '</script>' . "\n";
		...
		$this->content .= "\t" . 'function addMarker(latlng,title,content,username,category,icon,currentmap) {' . "\n";
		...
		//$this->content .= "\t\t" . 'var html = \'<div style="text-align:left;width:' . $this->infoWindowWidth . 'px;" class="infoGmaps">\'+content+\'\'' . "\n";
		$this->content .= "\t\t" . 'var html = content' . "\n";
		...
		$this->content .= "\t\t\t" . 'callUserData(username);' . "\n";
		$this->content .= "\t\t" . '});' . "\n";
		$this->content .= "\t\t" . 'marker.mycategory = category;' . "\n";
		$this->content .= "\t\t" . 'marker.username = username;' . "\n";
		$this->content .= "\t\t" . 'marker.content = content;' . "\n";
	...
	public function generate() {
		...
		$this->content .= "\t" . 'centerToCountry("' . $this->customLocationLat . '", "' . $this->customLocationLng . '", "' . $this->customViewportNLat . '", "' . $this->customViewportNLng . '", "' . $this->customViewportSLat . '", "' . $this->customViewportSLng . '");' . "\n";
		...
		if($this->defaultMarker != '')
			$this->content .= 'markerZoomToName("' . $this->defaultMarker . '")' . "\n";
		$this->content .= '}' . "\n";
